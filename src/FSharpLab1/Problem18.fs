...
let recursive () =
    ...
    let rec findMax row col =
        if row >= Array.length triangle then
            0
        else
...
let functional () =
    ...
    |> Array.reduce (fun lower upper ->
        Array.init (Array.length upper) (fun i -> upper.[i] + max lower.[i] lower.[i + 1]))
    |> Array.head
...
let sequence () =
    ...
    let folder currentRow previousRow =
        Seq.zip3 (Seq.append currentRow (seq { yield 0 })) (Seq.append (seq { yield 0 }) currentRow) previousRow
        |> Seq.map (fun (left, right, value) -> max left right + value)
        |> Seq.toArray

    triangle |> Array.rev |> Array.reduce folder |> Array.head
